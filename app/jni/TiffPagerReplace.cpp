...
JNIEXPORT jboolean JNICALL Java_org_beyka_tiffbitmapfactory_TiffReplace_replace(JNIEnv *env, jclass clazz, jstring filePath, jstring destFilePath,jstring resultPath,jint index) {
    ...
    *mp = '\0';
    ...
    TIFFSetDirectory(in,dirCount);
        ...
        if (!tiffcp(inputFile, out)) {
            LOGIS("tiffcpor dir error",resultFilePath);
            TIFFClose(out);
            return JNI_FALSE;
        }
        if(!TIFFWriteDirectory(out)) {
            LOGIS("tiffcpor dir error2222",resultFilePath);
            TIFFClose(out);
            return JNI_FALSE;
        }
        LOGII("tiffcpor dir ok count:", dirCount);
...
DECLAREcpFunc(cpDecodedStrips)
{
    ...
    if (buf) {
        ...
        uint32 row = 0;
        LOGII("tiffcpor td_strip ns", ns);
        for (s = 0; s < ns; s++) {
            tsize_t cc = (row + rowsperstrip > imagelength) ?
                         TIFFVStripSize(in, imagelength - row) : stripsize;
             //TIFFDirectory *td = &tif->tif_dir;
             // LOGII("tiffcpor td_stripsperimage ", td->td_stripsperimage);
                LOGII("tiffcpor td_strip ", s);
                LOGII("tiffcpor size ", cc);
            if (TIFFReadEncodedStrip(in, s, buf, cc) < 0
                && !ignore) {
                TIFFError(TIFFFileName(in),
                          "Error, can't read strip %lu",
                          (unsigned long) s);
                          LOGIS("tiffcpor ", "Error, can't read strip" );
                goto bad;
            }
            if (TIFFWriteEncodedStrip(out, s, buf, cc) < 0) {
                TIFFError(TIFFFileName(out),
                          ...
                          (unsigned long) s);
                          LOGIS("tiffcpor ", "Error, can't write strip" );
                goto bad;
            }
            row += rowsperstrip;
        }
    ...
    LOGIS("tiffcpor ", "can't allocate memory buffer of size" );
        TIFFError(TIFFFileName(in),
...
pickCopyFunc(TIFF* in, TIFF* out, uint16 bitspersample, uint16 samplesperpixel)
{
    ...
    if (!(TIFFIsTiled(out) || TIFFIsTiled(in))) {
        ...
        if (bias) {
            ...
            LOGIS("tiffcpor ", "Cannot handle tiled configuration w/bias image");
            return (NULL);
        }
        if (TIFFIsTiled(out)) {
...
#undef T
     ...
     LOGIS("tiffcpor ", "Don't know how to copy/convert image");
    return (NULL);
}
